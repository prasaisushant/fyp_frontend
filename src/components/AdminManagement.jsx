...
export default function AdminManagement() {
    ...
    const fetchAdmins = async () => {
        try {
            const response = await fetch('http://10.200.16.68:30582/api/admins', {
                headers: {
    ...
    const createAdmin = async (e) => {
        e.preventDefault();
        try {
            const response = await fetch('http://10.200.16.68:30582/signup', {
                method: 'POST',
                headers: {
    ...
    const deleteAdmin = async (username) => {
        try {
            await fetch(`http://10.200.16.68:30582/api/admins/${username}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Basic ${btoa(`${user.username}:${localStorage.getItem('password')}`)}`
                }
